...
DROP PROCEDURE IF EXISTS update_call_dates;

-- Create procedure for updating calls dates 
DELIMITER $$
CREATE PROCEDURE update_call_dates()
BEGIN
	DECLARE counter INT DEFAULT 1;
    DECLARE randomDate DATE;
    
    WHILE counter <= 25 DO
		SET randomDate = date_format(date_sub(now(), interval floor(rand() * 30) day), '%Y-%m-%d 00:00:00');
		UPDATE cfl_calls 
			SET startTime = randomDate, 
				endTime = randomDate, 
                date_created = randomDate, 
                date_changed = randomDate  
		WHERE cfl_calls_id = counter;
        SET counter = counter + 1;
    END WHILE;
END$$
DELIMITER ;

CALL update_call_dates(); 